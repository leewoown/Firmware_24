...
void InitAdc(void)
{
    ...
    AdcRegs.ADCSOC3CTL.bit.ACQPS  = 6; //Set SOC3 acquisition period to 7 ADCCLK

    AdcRegs.ADCSOC4CTL.bit.CHSEL  = 4; //Set SOC3 channel select to ADCINA3
    AdcRegs.ADCSOC4CTL.bit.ACQPS  = 6; //Set SOC3 acquisition period to 7 ADCCLK

    AdcRegs.ADCSOC4CTL.bit.CHSEL  = 7; //Set SOC3 channel select to ADCINA3
    AdcRegs.ADCSOC4CTL.bit.ACQPS  = 6; //Set SOC3 acquisition period to 7 ADCCLK

    // Setup ADCINT1 and ADCINT2 trigger source
    AdcRegs.INTSEL1N2.bit.INT1SEL = 14;      //EOC6 triggers ADCINT1
    AdcRegs.INTSEL1N2.bit.INT2SEL = 14;      //EOC14 triggers ADCINT2


//    AdcRegs.ADCOFFTRIM.bit.OFFTRIM=181;
...
/* AdcoffsetSelfCal-
   ...
   - AdcChanSelect(channel) �뜝占� selects the ADC channel to convert
   - AdcConversion() �뜝占� initiates several ADC conversions and returns the average
*/
